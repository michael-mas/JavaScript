...
window.addEventListener('load', function(){
    ...
    class InputHandler {
        constructor(){
            ...
            window.addEventListener('keydown', e => { // Voir ES6 arrow functions
                ...
                } else if(e.key === 'Enter' && gameOver) restartGame() //Si gameOver=True alors la touche entré active restartGame
            });
            ...
            window.addEventListener('touchstart', e => { // detecter mouvements quand on touche l'ecran: faire console.log(e) pour verifier valeurs de l'état swap mobile
                // faire des console.log pour recuperer les infos du tableaux de l'evenement Touch
    ...
    class Player {
        constructor(gameWidth, gameHeight){ // l'objet Player doit etre au courant des frontieres du jeu
            ...
            this.maxFrame = 48;
            this.frameY = 0;
            this.fps = 100; //affecte la navigation des sprite de façon horizontal
            this.frameTimer = 0;
        ...
        update(input, deltaTime, enemies){ // connecter mouvement aux touches de contrôle par l'argument input
            ...
            enemies.forEach(enemy => {
                ...
                if (distance < enemy.width/3 + this.width/3){//Si la distance est egale au rayon du cercle de l'enemie + le notre
            ...
            if (input.keys.indexOf('ArrowRight') > -1){ //Si appel de arrowUp via le tableau keys est lancé quand l'index est à -1 lance le code de la touche
                ...
                this.speed = -5;
            } else if ((input.keys.indexOf('ArrowUp') > -1 || input.keys.indexOf('swipe up') > -1) && this.onGround()){ // && Empeche le double saut
                this.vy -= 32; // vélocité
            } else {
    ...
    class Background {
        ...
        restart(){
            this.x = 0; //retour à zero au moment relancer partie
        }
    }
    ...
    class Enemy {
        constructor(gameWidth, gameHeight){ //definir parametre enemies
            ...
            this.frameX = 0;
            this.maxFrame = 20;
            this.fps = 40; //affecte la navigation des sprite de façon horizontal
            this.frameTimer = 0;
    ...
    function displayStatusText(context){
        ...
        if (gameOver){ //affiche un message en cas de gameOver = True
            ...
            context.fillStyle = 'black';
            context.fillText('PARTIE TERMINEE: ENTREE ou swipe vers le bas pour recommencer!', canvas.width/2, 200);
            context.fillStyle = 'white';
            context.fillText('PARTIE TERMINEE: ENTREE ou swipe vers le bas pour recommencer!', canvas.width/2 + 2, 202);
        }
    }

    //Pour relancer partie: rappel les objet et les met à l'etat d'oringe de la boucle
    ...
    //fonction pour appelé le mode plein écran via API fullscreenElement
    function toggleFullScreen(){
        console.log(document.fullscreenElement);
        if (!document.fullscreenElement){
            canvas.requestFullscreen().catch(err => {
                alert(`Erreur, impossible d'activer le mode plein écran: ${err.message}`) //Les littéraux de gabarits sont des littéraux de chaînes de caractères permettant d'intégrer des expressions.
